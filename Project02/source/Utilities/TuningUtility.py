...
class TuningUtility:
    ...
    def tune_sigma_k_and_epsilon_for_folds(self, training_test_data, tuning_data, sigma_range, sigma_step,
                                           epsilon_range, epsilon_step, k_range=None, train=False):
        ...
        for epsilon in numpy.arange(epsilon_range[0], epsilon_range[1] + epsilon_step, epsilon_step):
            for sigma in numpy.arange(sigma_range[0], sigma_range[1] + sigma_step, sigma_step):
                ...
                results = self.tune_k_for_folds(training_test_data, tuning_data, model_params=[sigma, epsilon],
                                                k_range=k_range, train=train)
    ...
    def tune_sigma_and_k_for_folds(self, training_test_data, tuning_data: pd.DataFrame, sigma_range, sigma_step,
                                   train=False, clusters=None, k_range=None):
        # Initialize the dictionary of all results.
        # Key = sigma
        # Value = list of data frames containing the k, mse, and trained model
        all_results = dict()
        ...
        for sigma in numpy.arange(sigma_range[0], sigma_range[1] + sigma_step, sigma_step):
            ...
            results = self.tune_k_for_folds(training_test_data, tuning_data, model_params=[sigma], train=train, k_range=k_range)
    ...
    def tune_k_for_folds(self, training_test_data, tuning_data: pd.DataFrame, model_params=[], k_range=None,
        ...
        if clusters is None:
            clusters = [None] * len(training_test_data)
        best_ks = []
        best_results = dict()
        ...
        for fold, (training_data, _, norm_params) in enumerate(training_test_data, start=1):
            ...
            results = self.tune_k_for_single_fold(training_data, tuning_data.copy(), norm_params=norm_params,
                                                  model_params=model_params, k_range=k_range, train=train,
                                                  cluster=clusters[fold - 1])
    ...
    def tune_k_for_single_fold(self, training_data: pd.DataFrame, test_data: pd.DataFrame,
                               norm_params: pd.DataFrame = None, model_params=[], k_range=None, train=None,
                               cluster=None):
        # If there isn't a defined range for the k values, use a default baseline
        if k_range is None:
            # [1, sqrt(n)]
